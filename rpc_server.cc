...
void 
RpcServer::SendErrorResponse(lws *wsi, int error_code, const char *error_text)
{
...
void
RpcServer::ProcessJsonRequest(const cJSON *json_object)
{  
...
void
RpcServer::ProcessProtobufRequest(const rdk::rpc::ProtocolDataUnit &pdu)
{
}

bool
RpcServer::IsRunning() const
{
...
int
RpcServer::lws_event_handler(lws *wsi, lws_callback_reasons reason, void *user, void *in, size_t len)
{   
...
void
RpcServer::OnConnectionEstablished(lws *wsi, SessionData* session_data)
{  
  std::unique_lock<std::mutex> lock(m_mutex);
  auto itr = m_response_queue.find(wsi);
  if (itr == std::end(m_response_queue)) {
...
void
RpcServer::OnConnectionClosed(lws *wsi, SessionData* session_data)
{  
  std::unique_lock<std::mutex> lock(m_mutex);
  auto itr = m_response_queue.find(wsi);
  if (itr != std::end(m_response_queue)) {
...
void
RpcServer::OnMessageReceived(lws *wsi, SessionData *session_data, void *in, size_t len)
{
...
void
RpcServer::OnClientWriteable(lws *wsi, SessionData* session_data)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  auto itr = m_response_queue.find(wsi);
  if (itr == std::end(m_response_queue)) {
...
void
RpcServer::EventHandler(lws *wsi, SessionData *session_data, lws_callback_reasons reason, void *in, size_t len)
{
  switch (reason) {
...
void 
RpcServer::RegisterLocalHostedService(const std::string& shared_object_name, const std::string& service_name)
{
...
void
RpcServer::RegisterLocalHostedService(google::protobuf::Service* service)
{
